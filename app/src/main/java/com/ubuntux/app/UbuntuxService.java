...
import com.ubuntux.app.terminal.UbuntuxTerminalSessionServiceClient;
...
import com.ubuntux.shared.shell.command.runner.app.AppShell;
...
import com.ubuntux.shared.shell.command.ExecutionCommand.ShellCreateMode;
import com.ubuntux.terminal.TerminalEmulator;
...
/**
 * A service holding a list of {@link UbuntuxSession} in {@link UbuntuxShellManager#mTermuxSessions} and background {@link AppShell}
 * in {@link UbuntuxShellManager#mTermuxTasks}, showing a foreground notification while running so that it is not terminated.
 * The user interacts with the session through {@link UbuntuxActivity}, but this service may outlive
 * the activity when the user or the system disposes of the activity. In that case the user may
 * restart {@link UbuntuxActivity} later to yet again access the sessions.
 * <p/>
 ...
 */
public final class UbuntuxService extends Service implements AppShell.AppShellClient, UbuntuxSession.TermuxSessionClient {

    /** This service is only bound from inside the same process and never uses IPC. */
    class LocalBinder extends Binder {
        public final UbuntuxService service = UbuntuxService.this;
    }
     ...
     */
    private UbuntuxTerminalSessionActivityClient mTermuxTerminalSessionActivityClient;

    /** The basic implementation of the {@link TerminalSessionClient} interface to be used by {@link TerminalSession}
     * that does not hold activity references and only a service reference.
     */
    private final UbuntuxTerminalSessionServiceClient mTermuxTerminalSessionServiceClient = new UbuntuxTerminalSessionServiceClient(this);

    /**
     * Ubuntux app shared properties manager, loaded from termux.properties
     */
    private UbuntuxAppSharedProperties mProperties;

    /**
     * Ubuntux app shell manager
     */
    private UbuntuxShellManager mShellManager;

    /** The wake lock and wifi lock are always acquired and released together. */
    private PowerManager.WakeLock mWakeLock;
    private WifiManager.WifiLock mWifiLock;

    /** If the user has executed the {@link UBUNTUX_SERVICE#ACTION_STOP_SERVICE} intent. */
    boolean mWantsToStop = false;

    private static final String LOG_TAG = "UbuntuxService";

    @Override
    public void onCreate() {
        Logger.logVerbose(LOG_TAG, "onCreate");

        // Get Termux app SharedProperties without loading from disk since UbuntuxApplication handles
        // load and UbuntuxActivity handles reloads
        mProperties = UbuntuxAppSharedProperties.getProperties();

        mShellManager = UbuntuxShellManager.getShellManager();
    ...
    public void onDestroy() {
        ...
        UbuntuxShellUtils.clearTermuxTMPDIR(true);

        actionReleaseWakeLock(false);
        if (!mWantsToStop)
            killAllTermuxExecutionCommands();

        UbuntuxShellManager.onAppExit(this);
    ...
    public boolean onUnbind(Intent intent) {
        ...
        // Since we cannot rely on {@link UbuntuxActivity.onDestroy()} to always complete,
    ...
    private void runStartForeground() {
        ...
        startForeground(UbuntuxConstants.UBUNTUX_APP_NOTIFICATION_ID, buildNotification());
    }
     ...
     * since once UbuntuxService has been killed, no result will be sent back. They may still get
    ...
    private synchronized void killAllTermuxExecutionCommands() {
        ...
        List<UbuntuxSession> termuxSessions = new ArrayList<>(mShellManager.mTermuxSessions);
    ...
    private void actionAcquireWakeLock() {
        ...
        mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, UbuntuxConstants.UBUNTUX_APP_NAME.toLowerCase() + ":service-wakelock");
        mWakeLock.acquire();

        // http://tools.android.com/tech-docs/lint-in-studio-2-3#TOC-WifiManager-Leak
        WifiManager wm = (WifiManager) getApplicationContext().getSystemService(Context.WIFI_SERVICE);
        mWifiLock = wm.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, UbuntuxConstants.UBUNTUX_APP_NAME.toLowerCase());
        mWifiLock.acquire();
    ...
    /** Process {@link UBUNTUX_SERVICE#ACTION_SERVICE_EXECUTE} intent to execute a shell command in
     * a foreground UbuntuxSession or in a background TermuxTask. */
    private void actionServiceExecute(Intent intent) {
        ...
        ExecutionCommand executionCommand = new ExecutionCommand(UbuntuxShellManager.getNextShellId());
        ...
        executionCommand.runner = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_RUNNER,
            (intent.getBooleanExtra(UBUNTUX_SERVICE.EXTRA_BACKGROUND, false) ? Runner.APP_SHELL.getName() : Runner.TERMINAL_SESSION.getName()));
        ...
        if (executionCommand.executableUri != null) {
            ...
            executionCommand.arguments = IntentUtils.getStringArrayExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_ARGUMENTS, null);
            if (Runner.APP_SHELL.equalsRunner(executionCommand.runner))
                executionCommand.stdin = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_STDIN, null);
            executionCommand.backgroundCustomLogLevel = IntentUtils.getIntegerExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_BACKGROUND_CUSTOM_LOG_LEVEL, null);
        }

        executionCommand.workingDirectory = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_WORKDIR, null);
        executionCommand.isFailsafe = intent.getBooleanExtra(UBUNTUX_ACTIVITY.EXTRA_FAILSAFE_SESSION, false);
        executionCommand.sessionAction = intent.getStringExtra(UBUNTUX_SERVICE.EXTRA_SESSION_ACTION);
        executionCommand.shellName = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_SHELL_NAME, null);
        executionCommand.shellCreateMode = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_SHELL_CREATE_MODE, null);
        executionCommand.commandLabel = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_COMMAND_LABEL, "Execution Intent Command");
        executionCommand.commandDescription = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_COMMAND_DESCRIPTION, null);
        executionCommand.commandHelp = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_COMMAND_HELP, null);
        executionCommand.pluginAPIHelp = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_PLUGIN_API_HELP, null);
        executionCommand.resultConfig.resultPendingIntent = intent.getParcelableExtra(UBUNTUX_SERVICE.EXTRA_PENDING_INTENT);
        executionCommand.resultConfig.resultDirectoryPath = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_RESULT_DIRECTORY, null);
        if (executionCommand.resultConfig.resultDirectoryPath != null) {
            executionCommand.resultConfig.resultSingleFile = intent.getBooleanExtra(UBUNTUX_SERVICE.EXTRA_RESULT_SINGLE_FILE, false);
            executionCommand.resultConfig.resultFileBasename = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_RESULT_FILE_BASENAME, null);
            executionCommand.resultConfig.resultFileOutputFormat = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_RESULT_FILE_OUTPUT_FORMAT, null);
            executionCommand.resultConfig.resultFileErrorFormat = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_RESULT_FILE_ERROR_FORMAT, null);
            executionCommand.resultConfig.resultFilesSuffix = IntentUtils.getStringExtraIfSet(intent, UBUNTUX_SERVICE.EXTRA_RESULT_FILES_SUFFIX, null);
        }
    ...
    public AppShell createTermuxTask(String executablePath, String[] arguments, String stdin, String workingDirectory) {
        return createTermuxTask(new ExecutionCommand(UbuntuxShellManager.getNextShellId(), executablePath,
    ...
    public synchronized AppShell createTermuxTask(ExecutionCommand executionCommand) {
        ...
        AppShell newTermuxTask = AppShell.execute(this, executionCommand, this,
            new UbuntuxShellEnvironment(), null,false);
        if (newTermuxTask == null) {
    ...
    /** Execute a shell command in a foreground {@link UbuntuxSession}. */
    private void executeTermuxSessionCommand(ExecutionCommand executionCommand) {
        if (executionCommand == null) return;

        Logger.logDebug(LOG_TAG, "Executing foreground \"" + executionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession command");

        // Transform executable path to shell/session name, e.g. "/bin/do-something.sh" => "do-something.sh".
        if (executionCommand.shellName == null && executionCommand.executable != null)
            executionCommand.shellName = ShellUtils.getExecutableBasename(executionCommand.executable);

        UbuntuxSession newTermuxSession = null;
        ShellCreateMode shellCreateMode = processShellCreateMode(executionCommand);
        if (shellCreateMode == null) return;
        if (ShellCreateMode.NO_SHELL_WITH_NAME.equals(shellCreateMode)) {
            newTermuxSession = getTermuxSessionForShellName(executionCommand.shellName);
            if (newTermuxSession != null)
                Logger.logVerbose(LOG_TAG, "Existing UbuntuxSession with \"" + executionCommand.shellName + "\" shell name found for shell create mode \"" + shellCreateMode.getMode() + "\"");
            else
                Logger.logVerbose(LOG_TAG, "No existing UbuntuxSession with \"" + executionCommand.shellName + "\" shell name found for shell create mode \"" + shellCreateMode.getMode() + "\"");
        }
        ...
        handleSessionAction(DataUtils.getIntFromString(executionCommand.sessionAction,
            UBUNTUX_SERVICE.VALUE_EXTRA_SESSION_ACTION_SWITCH_TO_NEW_SESSION_AND_OPEN_ACTIVITY),
            newTermuxSession.getTerminalSession());
    }

    /**
     * Create a {@link UbuntuxSession}.
     * Currently called by {@link UbuntuxTerminalSessionActivityClient#addNewSession(boolean, String)} to add a new {@link UbuntuxSession}.
     */
    @Nullable
    public UbuntuxSession createTermuxSession(String executablePath, String[] arguments, String stdin,
    ...
    /** Create a {@link UbuntuxSession}. */
    @Nullable
    public synchronized UbuntuxSession createTermuxSession(ExecutionCommand executionCommand) {
        if (executionCommand == null) return null;

        Logger.logDebug(LOG_TAG, "Creating \"" + executionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession");
        ...
        UbuntuxSession newTermuxSession = UbuntuxSession.execute(this, executionCommand, getTermuxTerminalSessionClient(),
            this, new UbuntuxShellEnvironment(), null, executionCommand.isPluginExecutionCommand);
        if (newTermuxSession == null) {
            Logger.logError(LOG_TAG, "Failed to execute new UbuntuxSession command for:\n" + executionCommand.getCommandIdAndLabelLogString());
            // If the execution command was started for a plugin, then process the error
            if (executionCommand.isPluginExecutionCommand)
                UbuntuxErrorUtils.processExecutionCommandError(this, LOG_TAG, executionCommand, false);
            else {
        ...
        // Notify {@link UbuntuxSessionsListViewController} that sessions list has been updated if
        ...
        UbuntuxActivity.updateUbuntuxActivityStyling(this, false);

        return newTermuxSession;
    }

    /** Remove a UbuntuxSession. */
    ...
    /** Callback received when a {@link UbuntuxSession} finishes. */
    @Override
    public void onTermuxSessionExited(final UbuntuxSession termuxSession) {
        if (termuxSession != null) {
            ExecutionCommand executionCommand = termuxSession.getExecutionCommand();

            Logger.logVerbose(LOG_TAG, "The onTermuxSessionExited() callback called for \"" + executionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession command");
    ...
    private void handleSessionAction(int sessionAction, TerminalSession newTerminalSession) {
        ...
        switch (sessionAction) {
            case UBUNTUX_SERVICE.VALUE_EXTRA_SESSION_ACTION_SWITCH_TO_NEW_SESSION_AND_OPEN_ACTIVITY:
                setCurrentStoredTerminalSession(newTerminalSession);
                if (mTermuxTerminalSessionActivityClient != null)
                    mTermuxTerminalSessionActivityClient.setCurrentSession(newTerminalSession);
                startUbuntuxActivity();
                break;
            case UBUNTUX_SERVICE.VALUE_EXTRA_SESSION_ACTION_KEEP_CURRENT_SESSION_AND_OPEN_ACTIVITY:
                if (getTermuxSessionsSize() == 1)
                    setCurrentStoredTerminalSession(newTerminalSession);
                startUbuntuxActivity();
                break;
            case UBUNTUX_SERVICE.VALUE_EXTRA_SESSION_ACTION_SWITCH_TO_NEW_SESSION_AND_DONT_OPEN_ACTIVITY:
                setCurrentStoredTerminalSession(newTerminalSession);
                if (mTermuxTerminalSessionActivityClient != null)
                    mTermuxTerminalSessionActivityClient.setCurrentSession(newTerminalSession);
                break;
            case UBUNTUX_SERVICE.VALUE_EXTRA_SESSION_ACTION_KEEP_CURRENT_SESSION_AND_DONT_OPEN_ACTIVITY:
                if (getTermuxSessionsSize() == 1)
                    setCurrentStoredTerminalSession(newTerminalSession);
                break;
            default:
                Logger.logError(LOG_TAG, "Invalid sessionAction: \"" + sessionAction + "\". Force using default sessionAction.");
                handleSessionAction(UBUNTUX_SERVICE.VALUE_EXTRA_SESSION_ACTION_SWITCH_TO_NEW_SESSION_AND_OPEN_ACTIVITY, newTerminalSession);
                break;
        }
    }

    /** Launch the {@link }UbuntuxActivity} to bring it to foreground. */
    private void startUbuntuxActivity() {
        ...
        if (PermissionUtils.validateDisplayOverOtherAppsPermissionForPostAndroid10(this, true)) {
            UbuntuxActivity.startUbuntuxActivity(this);
        } else {
            UbuntuxAppSharedPreferences preferences = UbuntuxAppSharedPreferences.build(this);
    ...
    /** If {@link UbuntuxActivity} has not bound to the {@link UbuntuxService} yet or is destroyed, then
     * interface functions requiring the activity should not be available to the terminal sessions,
     * so we just return the {@link #mTermuxTerminalSessionServiceClient}. Once {@link UbuntuxActivity} bind
     * callback is received, it should call {@link #setTermuxTerminalSessionClient} to set the
     * {@link UbuntuxService#mTermuxTerminalSessionActivityClient} so that further terminal sessions are directly
     * passed the {@link UbuntuxTerminalSessionActivityClient} object which fully implements the
     * {@link TerminalSessionClient} interface.
     *
     * @return Returns the {@link UbuntuxTerminalSessionActivityClient} if {@link UbuntuxActivity} has bound with
     * {@link UbuntuxService}, otherwise {@link UbuntuxTerminalSessionServiceClient}.
     */
    public synchronized UbuntuxTerminalSessionClientBase getTermuxTerminalSessionClient() {
        if (mTermuxTerminalSessionActivityClient != null)
    ...
    /** This should be called when {@link UbuntuxActivity#onServiceConnected} is called to set the
     * {@link UbuntuxService#mTermuxTerminalSessionActivityClient} variable and update the {@link TerminalSession}
     * and {@link TerminalEmulator} clients in case they were passed {@link UbuntuxTerminalSessionServiceClient}
     * earlier.
     *
     * @param termuxTerminalSessionActivityClient The {@link UbuntuxTerminalSessionActivityClient} object that fully
     * implements the {@link TerminalSessionClient} interface.
     */
    public synchronized void setTermuxTerminalSessionClient(UbuntuxTerminalSessionActivityClient termuxTerminalSessionActivityClient) {
    ...
    /** This should be called when {@link UbuntuxActivity} has been destroyed and in {@link #onUnbind(Intent)}
     * so that the {@link UbuntuxService} and {@link TerminalSession} and {@link TerminalEmulator}
    ...
    private Notification buildNotification() {
        ...
        Notification.Builder builder =  NotificationUtils.geNotificationBuilder(this,
            UbuntuxConstants.UBUNTUX_APP_NOTIFICATION_CHANNEL_ID, priority,
            UbuntuxConstants.UBUNTUX_APP_NAME, notificationText, null,
        ...
        String newWakeAction = wakeLockHeld ? UBUNTUX_SERVICE.ACTION_WAKE_UNLOCK : UBUNTUX_SERVICE.ACTION_WAKE_LOCK;
    ...
    private synchronized void updateNotification() {
        if (mWakeLock == null && mShellManager.mTermuxSessions.isEmpty() && mShellManager.mTermuxTasks.isEmpty()) {
            ...
            ((NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE)).notify(UbuntuxConstants.UBUNTUX_APP_NOTIFICATION_ID, buildNotification());
        }
    }
    ...
    public synchronized List<UbuntuxSession> getTermuxSessions() {
        return mShellManager.mTermuxSessions;
    }

    @Nullable
    public synchronized UbuntuxSession getTermuxSession(int index) {
    ...
    @Nullable
    public synchronized UbuntuxSession getTermuxSessionForTerminalSession(TerminalSession terminalSession) {
    ...
    public synchronized UbuntuxSession getLastTermuxSession() {
    ...
    public synchronized UbuntuxSession getTermuxSessionForShellName(String name) {
        if (DataUtils.isNullOrEmpty(name)) return null;
        UbuntuxSession termuxSession;
        for (int i = 0, len = mShellManager.mTermuxSessions.size(); i < len; i++) {
