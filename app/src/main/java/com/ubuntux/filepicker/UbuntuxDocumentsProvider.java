 ...
 */
public class UbuntuxDocumentsProvider extends DocumentsProvider {

    private static final String LOG_TAG = "UbuntuxDocumentsProvider";
    ...
    public ParcelFileDescriptor openDocument(String documentId, String mode, @Nullable CancellationSignal signal) 
        ...
        if (isWrite) {
            ...
            try {
                ...
                if (UbuntuxConstants.UBUNTUX_PROPERTIES_FILE_PATHS_LIST.contains(canonicalPath) ||
                    UbuntuxConstants.UBUNTUX_FLOAT_PROPERTIES_FILE_PATHS_LIST.contains(canonicalPath)) {
    ...
    private String getDocIdForFile(File file) {
        ...
        if (path.equals(UbuntuxConstants.UBUNTUX_FILES_DIR_PATH)) {
            return ROOT_DOCUMENT_ID;
        }
        
        // For files within the Ubuntux directory, use relative path
        if (path.startsWith(UbuntuxConstants.UBUNTUX_FILES_DIR_PATH)) {
            return path.substring(UbuntuxConstants.UBUNTUX_FILES_DIR_PATH.length());
        }
    ...
    private File getFileForDocId(String documentId) throws FileNotFoundException {
        ...
        if (ROOT_DOCUMENT_ID.equals(documentId)) {
            target = UbuntuxConstants.UBUNTUX_FILES_DIR;
        } else {
            // documentId should be a path relative to UBUNTUX_FILES_DIR
            target = new File(UbuntuxConstants.UBUNTUX_FILES_DIR, documentId);
        }
        ...
        try {
            ...
            if (!canonicalPath.startsWith(UbuntuxConstants.UBUNTUX_FILES_DIR.getCanonicalPath())) {
