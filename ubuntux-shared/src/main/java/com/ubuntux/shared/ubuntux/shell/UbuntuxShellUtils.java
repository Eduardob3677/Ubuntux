...
import com.ubuntux.shared.file.filesystem.FileTypes;
...
public class UbuntuxShellUtils {

    private static final String LOG_TAG = "UbuntuxShellUtils";
            ...
            try (FileInputStream in = new FileInputStream(file)) {
                ...
                if (bytesRead > 4) {
                    if (buffer[0] == 0x7F && buffer[1] == 'E' && buffer[2] == 'L' && buffer[3] == 'F') {
                            ...
                            if (c == ' ' || c == '\n') {
                                if (builder.length() == 0) {
                                    ...
                                    if (shebangExecutable.startsWith("/usr") || shebangExecutable.startsWith("/bin")) {
                                        ...
                                        interpreter = UbuntuxConstants.UBUNTUX_BIN_PREFIX_DIR_PATH + "/" + binary;
                                    }
                                    break;
                                }
                            } else {
                        ...
                        interpreter = UbuntuxConstants.UBUNTUX_BIN_PREFIX_DIR_PATH + "/sh";
                    }
                }
            }
        } catch (IOException e) {
    ...
    /** Clear files under {@link UbuntuxConstants#UBUNTUX_TMP_PREFIX_DIR_PATH}. */
    public static void clearTermuxTMPDIR(boolean onlyIfExists) {
        ...
        if(onlyIfExists && !FileUtils.directoryFileExists(UbuntuxConstants.UBUNTUX_TMP_PREFIX_DIR_PATH, false))
            return;
        ...
        if (days < 0) {
            ...
            error = FileUtils.clearDirectory("$TMPDIR",
                FileUtils.getCanonicalPath(UbuntuxConstants.UBUNTUX_TMP_PREFIX_DIR_PATH, null));
            if (error != null) {
