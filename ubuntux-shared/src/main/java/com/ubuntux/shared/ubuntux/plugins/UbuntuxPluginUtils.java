package com.ubuntux.shared.ubuntux.plugins;
...
import com.ubuntux.shared.shell.command.result.ResultConfig;
import com.ubuntux.shared.shell.command.result.ResultData;
...
import com.ubuntux.shared.shell.command.result.ResultSender;
...
public class UbuntuxPluginUtils {

    private static final String LOG_TAG = "UbuntuxPluginUtils";
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            ...
            if (error != null) {
                ...
                sendPluginCommandErrorNotification(context, logTag, null,
                    ...
                    false, true, UbuntuxUtils.AppInfoMode.UBUNTUX_AND_CALLING_PACKAGE,true,
     ...
     *                          {@link UBUNTUX_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED}
     *                          is {@code false}.
     ...
     *
     * Otherwise if the {@link UBUNTUX_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED} is
     * enabled, then a flash and a notification will be shown for the error as well
     * on the {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME} channel instead of just logging
     * the error.
     *
     * @param context The {@link Context} for operations.
     * @param logTag The log tag to use for logging.
    ...
    public static void processPluginExecutionCommandError(final Context context, String logTag,
        ...
        sendPluginCommandErrorNotification(context, logTag, null,
            ResultData.getErrorsListMinimalString(resultData),
            ExecutionCommand.getExecutionCommandMarkdownString(executionCommand),
            forceNotification, true, UbuntuxUtils.AppInfoMode.UBUNTUX_AND_CALLING_PACKAGE, true,
            executionCommand.resultConfig.resultPendingIntent != null ? executionCommand.resultConfig.resultPendingIntent.getCreatorPackage(): null);
    }
    ...
    public static void setPluginResultPendingIntentVariables(ExecutionCommand executionCommand) {
        ...
        resultConfig.resultBundleKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE;
        resultConfig.resultStdoutKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDOUT;
        resultConfig.resultStdoutOriginalLengthKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDOUT_ORIGINAL_LENGTH;
        resultConfig.resultStderrKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDERR;
        resultConfig.resultStderrOriginalLengthKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDERR_ORIGINAL_LENGTH;
        resultConfig.resultExitCodeKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_EXIT_CODE;
        resultConfig.resultErrCodeKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_ERR;
        resultConfig.resultErrmsgKey = UBUNTUX_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_ERRMSG;
    }
    ...
    public static void setPluginResultDirectoryVariables(ExecutionCommand executionCommand) {
        ...
        resultConfig.resultDirectoryPath = UbuntuxFileUtils.getCanonicalPath(resultConfig.resultDirectoryPath, null, true);
        resultConfig.resultDirectoryAllowedParentPath = UbuntuxFileUtils.getMatchedAllowedTermuxWorkingDirectoryParentPathForPath(resultConfig.resultDirectoryPath);
    ...
    /**
     * Send a plugin error report notification for {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     ...
     *                          regardless of if pending intent is {@code null} or
     *                          {@link UbuntuxPreferenceConstants.UBUNTUX_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED}
     *                          is {@code false}.
    ...
    public static void sendPluginCommandErrorNotification(final Context currentPackageContext, String logTag,
        ...
        sendPluginCommandErrorNotification(currentPackageContext, logTag,
            ...
            forceNotification, showToast, UbuntuxUtils.AppInfoMode.UBUNTUX_AND_PLUGIN_PACKAGE, addDeviceInfo, null);
    }

    /**
     * Send a plugin error notification for {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
     * @param logTag The log tag to use for logging.
    ...
    public static void sendPluginCommandErrorNotification(Context currentPackageContext, String logTag,
        ...
        if (termuxPackageContext == null) {
            Logger.logWarn(LOG_TAG, "Ignoring call to sendPluginCommandErrorNotification() since failed to get \"" + UbuntuxConstants.UBUNTUX_PACKAGE_NAME + "\" package context from \"" + currentPackageName + "\" context");
            return;
        }
        ...
        if (title == null || title.toString().isEmpty())
            title = UbuntuxConstants.UBUNTUX_APP_NAME + " Plugin Execution Command Error";
        ...
        if (appInfoMode != null)
            reportString.append("\n\n").append(UbuntuxUtils.getAppInfoMarkdownString(currentPackageContext, appInfoMode,
    ...
    /**
     * Get {@link Notification.Builder} for {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
    ...
    public static Notification.Builder getPluginCommandErrorsNotificationBuilder(final Context currentPackageContext,
        ...
        return UbuntuxNotificationUtils.getTermuxOrPluginAppNotificationBuilder(
            ...
            UbuntuxConstants.UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID, Notification.PRIORITY_HIGH,
            title, notificationText, notificationBigText, contentIntent, deleteIntent, notificationMode);
    }

    /**
     * Setup the notification channel for {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} and
     * {@link UbuntuxConstants#UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param context The {@link Context} for operations.
     */
    public static void setupPluginCommandErrorsNotificationChannel(final Context context) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) return;
        NotificationUtils.setupNotificationChannel(context, UbuntuxConstants.UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID,
            UbuntuxConstants.UBUNTUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME, NotificationManager.IMPORTANCE_HIGH);
    }



    /**
     * Check if {@link UbuntuxConstants#PROP_ALLOW_EXTERNAL_APPS} property is not set to "true".
     *
    ...
    public static String checkIfAllowExternalAppsPolicyIsViolated(final Context context, String apiName) {
        ...
        UbuntuxAppSharedProperties mProperties = UbuntuxAppSharedProperties.getProperties();
        if (mProperties == null || !mProperties.shouldAllowExternalApps()) {
            errmsg = context.getString(R.string.error_allow_external_apps_ungranted, apiName,
                UbuntuxFileUtils.getUnExpandedTermuxPath(UbuntuxConstants.UBUNTUX_PROPERTIES_PRIMARY_FILE_PATH));
        }
