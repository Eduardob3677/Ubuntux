package com.ubuntux.shared.ubuntux.notification;
...
import com.ubuntux.shared.android.resource.ResourceUtils;
...
public class UbuntuxNotificationUtils {
    /**
     * Try to get the next unique notification id that isn't already being used by the app.
     *
     * Ubuntux app and its plugin must use unique notification ids from the same pool due to usage of android:sharedUserId.
    ...
    public synchronized static int getNextNotificationId(final Context context) {
        if (context == null) return UbuntuxPreferenceConstants.UBUNTUX_APP.DEFAULT_VALUE_KEY_LAST_NOTIFICATION_ID;

        UbuntuxAppSharedPreferences preferences = UbuntuxAppSharedPreferences.build(context);
        if (preferences == null) return UbuntuxPreferenceConstants.UBUNTUX_APP.DEFAULT_VALUE_KEY_LAST_NOTIFICATION_ID;

        int lastNotificationId = preferences.getLastNotificationId();

        int nextNotificationId = lastNotificationId + 1;
        while(nextNotificationId == UbuntuxConstants.UBUNTUX_APP_NOTIFICATION_ID || nextNotificationId == UbuntuxConstants.UBUNTUX_RUN_COMMAND_NOTIFICATION_ID) {
            nextNotificationId++;
        }

        if (nextNotificationId == Integer.MAX_VALUE || nextNotificationId < 0)
            nextNotificationId = UbuntuxPreferenceConstants.UBUNTUX_APP.DEFAULT_VALUE_KEY_LAST_NOTIFICATION_ID;
