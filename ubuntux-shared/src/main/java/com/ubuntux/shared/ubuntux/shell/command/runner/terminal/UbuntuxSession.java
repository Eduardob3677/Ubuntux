...
public class UbuntuxSession {
    ...
    private static final String LOG_TAG = "UbuntuxSession";

    private UbuntuxSession(@NonNull final TerminalSession terminalSession, @NonNull final ExecutionCommand executionCommand,
     ...
     *                        available in the {@link TermuxSessionClient#onTermuxSessionExited(UbuntuxSession)}
     ...
     * @return Returns the {@link UbuntuxSession}. This will be {@code null} if failed to start the execution command.
     */
    public static UbuntuxSession execute(@NonNull final Context currentPackageContext, @NonNull ExecutionCommand executionCommand,
        ...
        if (!executionCommand.setState(ExecutionCommand.ExecutionState.EXECUTING)) {
            ...
            UbuntuxSession.processTermuxSessionResult(null, executionCommand);
            return null;
        }
        ...
        Logger.logDebug(LOG_TAG, "Running \"" + executionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession");
        ...
        return new UbuntuxSession(terminalSession, executionCommand, termuxSessionClient, setStdoutOnExit);
    }

    /**
     * Signal that this {@link UbuntuxSession} has finished.  This should be called when
     ...
     * and then calls {@link #processTermuxSessionResult(UbuntuxSession, ExecutionCommand)} to process the result}.
     *
     */
    public void finish() {
        ...
        if (exitCode == 0)
            Logger.logDebug(LOG_TAG, "The \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession exited normally");
        else
            Logger.logDebug(LOG_TAG, "The \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession exited with code: " + exitCode);

        // If the execution command has already failed, like SIGKILL was sent, then don't continue
        if (mExecutionCommand.isStateFailed()) {
            Logger.logDebug(LOG_TAG, "Ignoring setting \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession state to ExecutionState.EXECUTED and processing results since it has already failed");
            return;
        }
        ...
        UbuntuxSession.processTermuxSessionResult(this, null);
    }

    /**
     * Kill this {@link UbuntuxSession} by sending a {@link OsConstants#SIGILL} to its {@link #mTerminalSession}
     * if its still executing.
     *
     * @param context The {@link Context} for operations.
     * @param processResult If set to {@code true}, then the {@link #processTermuxSessionResult(UbuntuxSession, ExecutionCommand)}
    ...
    public void killIfExecuting(@NonNull final Context context, boolean processResult) {
        ...
        if (mExecutionCommand.hasExecuted()) {
            Logger.logDebug(LOG_TAG, "Ignoring sending SIGKILL to \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession since it has already finished executing");
            return;
        }

        Logger.logDebug(LOG_TAG, "Send SIGKILL to \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession");
    ...
    /**
     * Process the results of {@link UbuntuxSession} or {@link ExecutionCommand}.
     *
     ...
     * then the {@link UbuntuxSession.TermuxSessionClient#onTermuxSessionExited(UbuntuxSession)}
     * callback will be called.
     *
     * @param termuxSession The {@link UbuntuxSession}, which should be set if
     ...
     */
    private static void processTermuxSessionResult(final UbuntuxSession termuxSession, ExecutionCommand executionCommand) {
        ...
        if (executionCommand.shouldNotProcessResults()) {
            Logger.logDebug(LOG_TAG, "Ignoring duplicate call to process \"" + executionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession result");
            return;
        }

        Logger.logDebug(LOG_TAG, "Processing \"" + executionCommand.getCommandIdAndLabelLogString() + "\" UbuntuxSession result");
    ...
    public interface TermuxSessionClient {
        ...
        /**
         * Callback function for when {@link UbuntuxSession} exits.
         *
         * @param termuxSession The {@link UbuntuxSession} that exited.
         */
        void onTermuxSessionExited(UbuntuxSession termuxSession);
