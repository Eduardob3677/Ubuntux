...
import com.ubuntux.shared.file.FileUtilsErrno;
...
public class UbuntuxFileUtils {

    private static final String LOG_TAG = "UbuntuxFileUtils";
    ...
    public static List<String> getExpandedTermuxPaths(List<String> paths) {
        ...
        for (int i = 0; i < paths.size(); i++) {
            expandedPaths.add(getExpandedUbuntuxPath(paths.get(i)));
        }
     ...
     */
    public static String getExpandedUbuntuxPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^\\$PREFIX$", UbuntuxConstants.UBUNTUX_PREFIX_DIR_PATH);
            path = path.replaceAll("^\\$PREFIX/", UbuntuxConstants.UBUNTUX_PREFIX_DIR_PATH + "/");
            path = path.replaceAll("^~/$", UbuntuxConstants.UBUNTUX_HOME_DIR_PATH);
            path = path.replaceAll("^~/", UbuntuxConstants.UBUNTUX_HOME_DIR_PATH + "/");
        }
    ...
    public static String getUnExpandedTermuxPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^" + Pattern.quote(UbuntuxConstants.UBUNTUX_PREFIX_DIR_PATH) + "/", "\\$PREFIX/");
            path = path.replaceAll("^" + Pattern.quote(UbuntuxConstants.UBUNTUX_HOME_DIR_PATH) + "/", "~/");
        }
     ...
     *                   {@link UbuntuxFileUtils#getExpandedUbuntuxPath(String)} before its passed to
    ...
    public static String getCanonicalPath(String path, final String prefixForNonAbsolutePath, final boolean expandPath) {
        ...
        if (expandPath)
            path = getExpandedUbuntuxPath(path);
     ...
     * @return Returns the allowed path if it {@code path} is under it, otherwise {@link UbuntuxConstants#UBUNTUX_FILES_DIR_PATH}.
     */
    public static String getMatchedAllowedTermuxWorkingDirectoryParentPathForPath(String path) {
        if (path == null || path.isEmpty()) return UbuntuxConstants.UBUNTUX_FILES_DIR_PATH;

        if (path.startsWith(UbuntuxConstants.UBUNTUX_STORAGE_HOME_DIR_PATH + "/")) {
            return UbuntuxConstants.UBUNTUX_STORAGE_HOME_DIR_PATH;
        } if (path.startsWith(Environment.getExternalStorageDirectory().getAbsolutePath() + "/")) {
            return Environment.getExternalStorageDirectory().getAbsolutePath();
        } else if (path.startsWith("/sdcard/")) {
            return "/sdcard";
        } else {
            return UbuntuxConstants.UBUNTUX_FILES_DIR_PATH;
        }
    }
    ...
    public static Error validateDirectoryFileExistenceAndPermissions(String label, final String filePath, final boolean createDirectoryIfMissing,
        ...
        return FileUtils.validateDirectoryFileExistenceAndPermissions(label, filePath,
            UbuntuxFileUtils.getMatchedAllowedTermuxWorkingDirectoryParentPathForPath(filePath), createDirectoryIfMissing,
    ...
    /**
     * Validate if {@link UbuntuxConstants#UBUNTUX_FILES_DIR_PATH} exists and has
     ...
     * {@link UbuntuxConstants#UBUNTUX_PREFIX_DIR_PATH} and the path must be accessible.
     *
     * The permissions set to directory will be {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS}.
     *
     ...
     * defined by default by {@link UbuntuxConstants#UBUNTUX_FILES_DIR_PATH} where id will be 0 for
     * primary user and a higher number for other users/profiles. If app is running under work profile
     * or secondary user, then {@link UbuntuxConstants#UBUNTUX_FILES_DIR_PATH} will not be accessible
    ...
    public static Error isTermuxFilesDirectoryAccessible(@NonNull final Context context, boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        ...
        if (!FileUtils.directoryFileExists(UbuntuxConstants.UBUNTUX_FILES_DIR_PATH, true))
            return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError("termux files directory", UbuntuxConstants.UBUNTUX_FILES_DIR_PATH);

        if (setMissingPermissions)
            FileUtils.setMissingFilePermissions("termux files directory", UbuntuxConstants.UBUNTUX_FILES_DIR_PATH,
                FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS);

        return FileUtils.checkMissingFilePermissions("termux files directory", UbuntuxConstants.UBUNTUX_FILES_DIR_PATH,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, false);
    }

    /**
     * Validate if {@link UbuntuxConstants#UBUNTUX_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     * .
     *
     * The {@link UbuntuxConstants#UBUNTUX_PREFIX_DIR_PATH} directory would not exist if termux has
    ...
    public static Error isTermuxPrefixDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
           return FileUtils.validateDirectoryFileExistenceAndPermissions("termux prefix directory", UbuntuxConstants.UBUNTUX_PREFIX_DIR_PATH,
    ...
    /**
     * Validate if {@link UbuntuxConstants#UBUNTUX_STAGING_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file
     *                                 should be created if its missing.
    ...
    public static Error isTermuxPrefixStagingDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        return FileUtils.validateDirectoryFileExistenceAndPermissions("termux prefix staging directory", UbuntuxConstants.UBUNTUX_STAGING_PREFIX_DIR_PATH,
            null, createDirectoryIfMissing,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, setMissingPermissions, true,
            false, false);
    }
    ...
    /**
     * Validate if {@link UbuntuxConstants.UBUNTUX_APP#APPS_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file
     *                                 should be created if its missing.
    ...
    public static Error isAppsTermuxAppDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        return FileUtils.validateDirectoryFileExistenceAndPermissions("apps/termux-app directory", UbuntuxConstants.UBUNTUX_APP.APPS_DIR_PATH,
            null, createDirectoryIfMissing,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, setMissingPermissions, true,
            false, false);
    }
    ...
    /**
     * If {@link UbuntuxConstants#UBUNTUX_PREFIX_DIR_PATH} doesn't exist, is empty or only contains
     * files in {@link UbuntuxConstants#UBUNTUX_PREFIX_DIR_IGNORED_SUB_FILES_PATHS_TO_CONSIDER_AS_EMPTY}.
     */
    public static boolean isTermuxPrefixDirectoryEmpty() {
        Error error = FileUtils.validateDirectoryFileEmptyOrOnlyContainsSpecificFiles("termux prefix",
            UBUNTUX_PREFIX_DIR_PATH, UbuntuxConstants.UBUNTUX_PREFIX_DIR_IGNORED_SUB_FILES_PATHS_TO_CONSIDER_AS_EMPTY, true);
        if (error == null)
            return true;
    ...
    public static String getTermuxFilesStatMarkdownString(@NonNull final Context context) {
        Context termuxPackageContext = UbuntuxUtils.getTermuxPackageContext(context);
        ...
        statScript
            ...
            .append(" '/data/data'")
            .append(" '/data/user/0'")
            .append(" '" + UbuntuxConstants.UBUNTUX_INTERNAL_PRIVATE_APP_DATA_DIR_PATH + "'")
            .append(" '/data/user/0/" + UbuntuxConstants.UBUNTUX_PACKAGE_NAME + "'")
            .append(" '" + UbuntuxConstants.UBUNTUX_FILES_DIR_PATH + "'")
            .append(" '" + filesDir + "'")
            .append(" '/data/user/0/" + UbuntuxConstants.UBUNTUX_PACKAGE_NAME + "/files'")
            .append(" '/data/user/" + UbuntuxConstants.UBUNTUX_PACKAGE_NAME + "/files'")
            .append(" '" + UbuntuxConstants.UBUNTUX_STAGING_PREFIX_DIR_PATH + "'")
            .append(" '" + UbuntuxConstants.UBUNTUX_PREFIX_DIR_PATH + "'")
            .append(" '" + UbuntuxConstants.UBUNTUX_HOME_DIR_PATH + "'")
            .append(" '" + UbuntuxConstants.UBUNTUX_BIN_PREFIX_DIR_PATH + "/login'")
            .append(" 2>&1")
        ...
        executionCommand.commandLabel = UbuntuxConstants.UBUNTUX_APP_NAME + " Files Stat Command";
        ...
        markdownString.append("## ").append(UbuntuxConstants.UBUNTUX_APP_NAME).append(" Files Info\n\n");
        AndroidUtils.appendPropertyToMarkdown(markdownString,"UBUNTUX_REQUIRED_FILES_DIR_PATH ($PREFIX)", UbuntuxConstants.UBUNTUX_FILES_DIR_PATH);
